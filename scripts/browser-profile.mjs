...
if (urls.length !== 2)
  throw new Error(
    "Usage: node scripts/browser-profile.mjs <baseline URL> <candidate URL>",
  );
...
try {
  for (let trial = 0; trial < 3; trial++)
    for (const [viewportName, width, height] of [
      ...
      for (const [version, url] of urls.entries()) {
            ...
            (ms) =>
              new Promise((resolve) => {
                ...
                function tick(now) {
                  ...
                  else resolve(frames.sort((a, b) => a - b));
                }
        ...
        results.push({
          ...
          version: version ? "candidate" : "baseline",
          readyMs,
          resources,
          overview,
          reading,
        ...
        console.log(
          `Trial ${trial + 1}: ${viewportName} ${version ? "candidate" : "baseline"} complete`,
        );
        await context.close();
      }
  const report = {
  ...
  await writeFile(
    "browser-results/performance.json",
